...
function ConfettiBurst({ offsetMs = 0 }) {
  ...
  return (
    <div className="gcConfetti" aria-hidden="true" style={{ "--base": `${offsetMs}ms` }}>
      {pieces.map((p, i) => (
        <span
          key={i}
          className="gcConfettiPiece"
          style={{
            "--x": `${p.x}px`,
            "--y": `${p.y}px`,
            "--r": `${p.r}deg`,
      ...
      <div className="gcBurstRing" />
      <div className="gcSparkle" />
    </div>
  );
}

export default function GoldCore() {
  ...
  // ✅ keep string
  const [investAmount, setInvestAmount] = useState(String(GOLD_CORE.min));
  ...
  const packagePrice = GOLD_CORE.min;
  ...
  function openSubscribeDefault() {
    setSubErr("");
    setInvestAmount(String(GOLD_CORE.min));
    setSubOpen(true);
  }
  ...
  function openSubscribeFromROI() {
    ...
    if (a < GOLD_CORE.min || a > GOLD_CORE.max) {
      setRoiErr(`Invalid data. Allowed range is $${money(GOLD_CORE.min)} – $${money(GOLD_CORE.max)}.`);
      return;
    }
  ...
  function validateInvest(aStr) {
    ...
    if (x < GOLD_CORE.min) return `Invalid data. Minimum investment is $${money(GOLD_CORE.min)}.`;
    if (x > GOLD_CORE.max) return `Invalid data. Maximum investment is $${money(GOLD_CORE.max)}.`;
  ...
  return (
    <div className="gcPage">
      <div className="gcBg" aria-hidden="true" />
      ...
      <div className="gcContainer">
        ...
        <div className="gcHeader">
          <div>
            ...
            <div className="gcKicker">Mining Package</div>
            <h1 className="gcTitle">{GOLD_CORE.name}</h1>
            <p className="gcSub">{GOLD_CORE.subtitle}</p>
          </div>

          <div className="gcHeaderActions">
            ...
            <div className="gcStatusPill">
              <span className={`gcDot ${USER.active ? "on" : "off"}`} />
        ...
        <section className="gcSummary">
          ...
          <div className="sumCard">
            ...
            <div className="sumValue gold">{pct(totalReturnPct)}</div>
            <div className="sumMeta">{GOLD_CORE.durationDays} days • Simple model</div>
          </div>
        </section>

        {/* Main Grid */}
        <section className="gcGrid">
          {/* Overview */}
          <div className="gcBlock">
            <div className="blockHead">
              <div>
                <div className="blockTitle">GoldCore Overview</div>
                <div className="blockHint">Premium plan terms & allocation info for this rig.</div>
              </div>
            ...
            <div className="overviewBody">
              <div className="rigCard">
                <div className="rigPhoto" style={{ backgroundImage: `url("${GOLD_CORE.image}")` }} />

                <div className="rigInfo">
                  <div className="rigName">{GOLD_CORE.name}</div>
                  <div className="rigDesc">
                    A premium-tier daily accrual package designed for higher yield allocations with stable settlement rules
                    (backend-defined).
                  </div>
                  ...
                  <div className="kpiRow">
                    <div className="kpiBox">
                      ...
                      <div className="kpiValue gold">{pct(GOLD_CORE.dailyPct)}</div>
                    </div>
                    <div className="kpiBox">
                      <div className="kpiLabel">Duration</div>
                      <div className="kpiValue">{GOLD_CORE.durationDays} days</div>
                    </div>
                    <div className="kpiBox">
                      <div className="kpiLabel">Total Return</div>
                      <div className="kpiValue gold">{pct(totalReturnPct)}</div>
                    </div>
                  </div>
                  ...
                  <div className="rows">
                    <div className="row">
                      ...
                      <span className="rowVal">${money(GOLD_CORE.min)}</span>
                    </div>
                    <div className="row">
                      <span className="rowKey">Maximum</span>
                      <span className="rowVal">${money(GOLD_CORE.max)}</span>
                    </div>
                    <div className="row">
                      <span className="rowKey">Settlement</span>
                      <span className="rowVal">{GOLD_CORE.settlement}</span>
                    </div>
                    <div className="row">
                      <span className="rowKey">Risk Notes</span>
                      <span className="rowVal muted">{GOLD_CORE.riskNote}</span>
                    </div>
                  </div>
            ...
            <div className="calcBody">
              <label className="field">
                ...
                <input
                  ...
                  placeholder={`Suggested: ${GOLD_CORE.min}`}
                />

                <div className={calc.inRange ? "help ok" : "help warn"}>
                  Range: ${money(GOLD_CORE.min)} – ${money(GOLD_CORE.max)}
                  {!calc.inRange && (
                    <span className="helpTag">{amount === "" ? "Required" : "Out of range"}</span>
                  )}
                </div>

                <div className="gcRoiTools">
                  <button
                    type="button"
                    className="gcSuggestPill"
                    onClick={() => setAmount(String(GOLD_CORE.min))}
                  >
                    Suggested: ${money(GOLD_CORE.min)}
                  </button>

                  {roiErr ? <div className="gcRoiError">{roiErr}</div> : null}
                </div>
              </label>
              ...
              <div className="calcGrid">
                <div className="calcCard">
                  ...
                  <div className="calcValue gold">${money(calc.dailyProfit)}</div>
                  <div className="calcMeta">({pct(GOLD_CORE.dailyPct)} of principal)</div>
                </div>
              ...
              <div className="calcActions">
                ...
                <button className="ghostBtn" onClick={() => setAmount(String(GOLD_CORE.min))}>
                  Use Minimum
                </button>
              </div>
      ...
      {/* Step 1 */}
      {subOpen && (
        <div className="gcModalOverlay" role="dialog" aria-modal="true">
          <div className="gcModal">
            <div className="gcModalHead">
              <div className="gcHeadLeft">
                <button className="gcBackIconBtn" onClick={closeAllModals} aria-label="Back">
                  <BackIcon />
                </button>
                <div className="gcModalTitle">Confirm Subscription</div>
              </div>

              <button className="gcModalX" onClick={closeAllModals} aria-label="Close">
                ×
              </button>
            </div>

            <div className="gcModalBody">
              <div className="gcModalDesc">
                You are about to buy <b>{GOLD_CORE.name}</b>. Minimum package price is shown below.
              </div>

              <div className="gcInfoGrid">
                <div className="gcInfoCard">
                  <div className="gcInfoLabel">Package Price (Minimum)</div>
                  <div className="gcInfoValue">${money(GOLD_CORE.min)}</div>
                </div>
                <div className="gcInfoCard">
                  <div className="gcInfoLabel">Your Current Balance</div>
                  <div className={`gcInfoValue ${canBuy ? "ok" : "warn"}`}>${money(balance)}</div>
                </div>
              </div>

              {!canBuy && (
                <div className="gcShortage">
                  <span className="gcShortageDot" />
                  Insufficient balance. Shortage: <b>${money(shortage)}</b>
                </div>
              )}

              {!!subErr && <div className="gcError">{subErr}</div>}
            </div>

            <div className="gcModalActions">
      ...
      {investOpen && (
        <div className="gcModalOverlay" role="dialog" aria-modal="true">
          <div className="gcModal">
            <div className="gcModalHead">
              <div className="gcHeadLeft">
                <button className="gcBackIconBtn" onClick={backToConfirm} aria-label="Back">
                  <BackIcon />
                </button>
                <div className="gcModalTitle">Enter Investment Amount</div>
              </div>
            ...
            <div className="gcModalBody">
              <div className="gcModalDesc">
                Choose how much you want to invest in <b>{GOLD_CORE.name}</b>.
              </div>

              <div className="gcInfoGrid">
                <div className="gcInfoCard">
                  <div className="gcInfoLabel">Allowed Range</div>
                  <div className="gcInfoValue">
                    ${money(GOLD_CORE.min)} – ${money(GOLD_CORE.max)}
                  </div>
                </div>
                <div className="gcInfoCard">
                  <div className="gcInfoLabel">Your Balance</div>
                  <div className="gcInfoValue ok">${money(balance)}</div>
                </div>
              </div>

              <label className="gcField">
                <div className="gcFieldLabel">Investment Amount (USD)</div>
                <input
                  className="gcFieldControl"
                  type="number"
                  inputMode="decimal"
                  min={GOLD_CORE.min}
                  max={GOLD_CORE.max}
                  ...
                  placeholder={`Min ${GOLD_CORE.min}`}
                />
                <div className="gcFieldHint">Must be within range and not exceed your balance.</div>
              </label>
      ...
      {loadingOpen && (
        <div className="gcModalOverlay" role="dialog" aria-modal="true">
          <div className="gcModal gcModalSm">
            <div className="gcModalHead">
              <div className="gcHeadLeft">
                <button className="gcBackIconBtn" onClick={closeAllModals} aria-label="Back">
                  <BackIcon />
                </button>
                <div className="gcModalTitle">Processing</div>
              </div>
            ...
            <div className="gcModalBody">
              <div className="gcLoaderWrap">
                <div className="gcSpinner" aria-hidden="true" />
                <div className="gcLoaderText">
                  Confirming your subscription…
                  <div className="gcLoaderSmall">Please wait {secondsLeft}s</div>
                </div>
              </div>

              <div className="gcProgress">
                <div className="gcProgressBar" style={{ width: `${((15 - secondsLeft) / 15) * 100}%` }} />
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="gcModal gcModalSm">
            ...
            <div className="gcModalHead">
              <div className="gcHeadLeft">
                ...
                <div className="gcModalTitle">Successful</div>
              </div>
            ...
            <div className="gcModalBody">
              <div className="gcCongrats">
                <div className="gcCongratsTitle">🎉 Congratulations!</div>
                <div className="gcCongratsMsg">
                  You have successfully activated the <b>{GOLD_CORE.name}</b> package.
                  <div className="gcCongratsSub">
                    Invested: <b>${money(investedNum)}</b> • Enjoy daily rewards and track your earnings anytime.
                  </div>
                </div>
              </div>

              <div className="gcSuccess">
                <div className="gcTick" aria-hidden="true">
                  <svg viewBox="0 0 24 24">
                    <path d="M20 6L9 17l-5-5" />
                  </svg>
                </div>

                <div className="gcSuccessText">
                  Subscription completed successfully!
                  <div className="gcSuccessSmall">
                    Invested: <b>${money(investedNum)}</b> • Package: <b>{GOLD_CORE.name}</b>
                  </div>
                </div>
              </div>
            </div>
